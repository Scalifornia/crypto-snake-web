...
- Description: Category -> subcategory -> specialty -> service listing model for P2P services. Taxonomy selectors should include an `Other` path where users can write a missing category, subcategory, profession, or specialty; these values are proposal/mock data for later admin review, not official taxonomy entries yet.
- Status: In Progress.
